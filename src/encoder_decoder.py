...
def cfg():
    ...
    train_fraction = 0.4 # fraction of the table to use in the train set
    encoder_width = 200
    encoder_depth = 3
...
def run(p,
    ...
    for step in tqdm(range(int(train_steps))):
        ...
        l = loss_fn(y_target, y_train).to(device)
        ...
        if step % int(log_freq) == 0:
            ...
            print((step, ex.info['train_accuracies'][-1], ex.info['test_accuracies'][-1]))
        # backprop and step
        l.backward()
        optim.step()
    ...
    from collections import defaultdict
    df = defaultdict(list)
    for test_acc, train_acc, step in zip(ex.info['test_accuracies'], ex.info['train_accuracies'], ex.info['steps']):
        df["step"].append(step)
        df["metric_name"].append("train_acc")
        df["metric_value"].append(train_acc)
        df["step"].append(step)
        df["metric_name"].append("test_acc")
        df["metric_value"].append(test_acc)
        
    
    df = pd.DataFrame.from_dict(df)

   
    lineplot(df, "step", "metric_value", hue = "metric_name", dict_legend_loss = None, hue_order = None, phase_label_positions = None, hide_legend = False, legend_title = None, show = True, 
             folder = None, filename = None, title = None, xlabel=None, ylabel=None, logscale=False, line_coords = [], show_ylabel = True)
    
    """
    torch.save(model.state_dict(), "./model-final-state-dict.pt")
    ex.add_artifact("./model-final-state-dict.pt", "model-final-state-dict.pt")
    torch.save((symbol_reps, train_pairs, test_pairs, train_data, test_data), "./dataset.pt")
    ex.add_artifact("/dataset.pt", "dataset.pt")
    """